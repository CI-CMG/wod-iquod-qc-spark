...
public class CsiroConstantBottomCheck extends CommonCastCheck {
  ...
  protected Collection<Integer> getFailedDepths(Cast cast) {
    ...
    Optional<Double> temp1 = depths.get(depths.size()-1).getData().stream()
        .filter(pd -> pd.getVariable()==TEMPERATURE).findFirst()
        .map(v-> v.getValue());

    Optional<Double> temp2 = depths.get(depths.size()-2).getData().stream()
        .filter(pd -> pd.getVariable()==TEMPERATURE).findFirst()
        .map(v-> v.getValue());
  ...
  private static boolean doubleEquals(Optional<Double> v1, Optional<Double> v2){
